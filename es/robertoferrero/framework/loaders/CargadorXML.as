...
package es.robertoferrero.framework.loaders {
	...
	//---------------------------
	// CLASSES del Package:
	public class CargadorXML extends EventDispatcher implements Cargador {
		...
		// CONSTRUCTORA: CargadorXML(_path:String,_cargaId:String=null, _buffer:Buffer=null, _dataEnvio:Object=null, _metodo:String="POST", _emitirEventosURLLoader=false)
		///------------
		// PARAMETROS:
		private var _path:String
		private var _cargaId:String
		private var buffer:Buffer
		private var _tipocarga:String = "CargadorXML"
		//--
		...
		//------------
		// CONSTRUCTORA:
		public function CargadorXML(path:String,cargaId:String=null, _buffer:Buffer=null, _dataEnvio:Object=null, _metodo:String="POST") {
			trace("(CargadorXML.CONSTRUCTORA) cargaId: "+_cargaId);
			_path=path;
			_cargaId=cargaId;
			buffer = _buffer
		...
		public function init():void {
			//trace("(CargadorXML.init) cargaId: "+cargaId);
			if (this.buffer == null) {
				cargar()
			}else {
		...
		private function cargar() {
			trace("(CargadorXML.cargar): " + cargaId)
		...
		public function onIniciarCarga(bufferEvent:EventoCarga):void { // Obligado por interface "Cargador"
			...
			if (cargaId == bufferEvent.cargaId) {
				trace("(CargadorXML.onIniciarCarga) cargadId: " + bufferEvent.cargaId);
				cargar();
			}
		}
		//------
		...
		private function onCargaIniciada(loaderEvent:Event):void {
			// Lo emite URLLoader
			trace("(CargadorXML.onCargaIniciada)  cargadId: " + cargaId);
		...
		private function onProgresoCarga(loaderEvent:ProgressEvent):void {
			// Lo emite URLLoader
			//trace("(CargadorXML.onProgresoCarga)  cargadId: " + cargaId+"   "+ event.bytesLoaded + "/" + event.bytesTotal);
		...
		private function onExitoCarga(loaderEvent:Event):void {
			// Lo emite URLLoader
			trace("(CargadorXML.onExitoCarga):" + cargaId);
			var dataXML:XML = new XML(loaderEvent.target.data)
			//--
			var evento:EventoCarga = new EventoCarga(EventoCarga.EXITO_CARGA, cargaId, filtrar_bufferId(), dataXML, loaderEvent);
			this.dispatchEvent(evento);
			//--
			if (emitirEventosURLLoader) {
				this.dispatchEvent(loaderEvent);
		...
		private function onSecurityError(loaderEvent:SecurityErrorEvent):void {
			// Lo emite URLLoader
			trace("(CargadorXML.onOpen):" + cargaId);
		...
		private function onHttpStatus(loaderEvent:HTTPStatusEvent):void {
			// Lo emite URLLoader
			//trace("(CargadorXML.onHttpStatus) : " + loaderEvent);
			if (emitirEventosURLLoader) {
				this.dispatchEvent(loaderEvent);
			}
		}
		private function onIoError(loaderEvent:IOErrorEvent):void {
			// Lo emite URLLoader
			trace("(CargadorXML.onIoError):" + cargaId);
			var evento:EventoCarga = new EventoCarga(EventoCarga.ERROR_CARGA, cargaId, filtrar_bufferId(), null, loaderEvent);
			this.dispatchEvent(evento);
			//--
			if (emitirEventosURLLoader) {
